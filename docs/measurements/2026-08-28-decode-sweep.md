...
*The following note was added by hand after the automated report above; it is not generated by `toMarkdown`.*

## Reading these numbers

- **`p16-fp16-b8-none-r1`'s budget (10453.7 ms) is inflated by a cold-start artifact, not a real cost.** It is the first fp16 row in grid order and its `model-load` (2948.9 ms) and `encode` (1468.5 ms) are far above every other fp16 row's (about 633-657 ms and 635-745 ms respectively) — it paid fp16 weight download and pipeline compilation that every later fp16 row got for free. Do not read its rank relative to other fp16/batch-8 rows (row 12, ranked below the fp32/batch-32 group) as a real difference; the sweep discards exactly one warm-up for the whole grid, not one per dtype, so whichever config is first of its kind in grid order can carry this artifact in a single-rep run.
- **The four decode paths' budgets are NOT uniformly within ~2% of each other** — that figure in the commit message understates the spread in at least one group. Within a matching dtype/batch group (excluding the fp16/batch-8 group, whose spread is dominated by the cold-start row above) the observed spread ranges from about 2.0% up to about 4.0%: fp32/batch-32 budgets span 10063.9-10470.2 ms, a 4.0% spread, and the fp32/batch-8 `decode+filter` sums the commit message itself tabulates (none 9007.2, overlap 9025.0, both 9101.1, gpuEmbeddings 9354.3) span 9007.2-9354.3, a 3.9% spread. The conclusion is unchanged — no decode path is a clear win — but the precise "~2%" figure in the commit message should not be quoted as a tight bound.
